...
export default class Favourite extends Component {
    constructor(props) {
        ...
        this.state = {
            applications: [
                { id: 1, name: 'John Doe', email: 'john.doe@example.com', subject: 'Software Engineer Application', position: 'Software Engineer' },
                { id: 2, name: 'Jane Smith', email: 'jane.smith@example.com', subject: 'Web Developer Application', position: 'Web Developer' }
    ...
    renderApplications() {
        return this.state.applications.map(application => (
            <View key={application.id} style={styles.application}>
                ...
                <TouchableOpacity style={styles.button} onPress={() => this.viewResume(application)}>
                    <Text style={styles.buttonText}>View Resume</Text>
                </TouchableOpacity>
                <TouchableOpacity style={styles.button} onPress={() => this.shortlist(application)}>
                    <Text style={styles.buttonText}>Shortlist</Text>
                </TouchableOpacity>
                <TouchableOpacity style={styles.button} onPress={() => this.reject(application)}>
